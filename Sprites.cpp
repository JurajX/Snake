...
int Part::FrontInt() { return static_cast<int>(front); }
...
,mTileSize(tile_size) {
    ...
    if (body_ud == nullptr or body_rl == nullptr or body_ur == nullptr or
        ...
        SDL_Log("Failed to load one of the apple images: %s", IMG_GetError());}
...
void Snake::BlitOnPlayground(SDL_Surface *playground_surface) {
    for (int i = 0; i < mBodyParts.size(); i++) {
        ...
        if (i == mBodyParts.size()-1) {
            SDL_BlitScaled(mHeadSurfaces[mBodyParts[i].FrontInt()], nullptr, playground_surface, &dstRect);
        } else if (i == 0) {
            SDL_BlitScaled(mBodySurfaces[mBodyParts[i].FrontInt()], nullptr, playground_surface, &dstRect);
        } else if (i > 0){
            SDL_BlitScaled(mBodySurfaces[mBodyParts[i].FrontInt()+mBodyParts[i-1].BackInt()], nullptr, playground_surface, &dstRect);
        }
    }
}
